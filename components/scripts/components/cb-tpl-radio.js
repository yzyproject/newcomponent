 ...
 * description: 单选框组件。
 */
 var CbTplRadio = (function () {
     //cb-tpl-radio组件结构列表
     var domDivList = {
       "default": '<div class="cb-tpl-radio-div">'+
                    '<ul class="cb-tpl-radio-list"></ul>'+
                  '</div>'
     ...
     var tplList = {
       "default": '{{each list as value i }}'+
                    '<li>'+
                      '<input class="tydic-radio" type="radio" name="{{options.name}}" value="{{value.value}}" id="{{options.cid+options._totalIndex+i}}" {{if value.disabled}} disabled="disabled" {{/if}} {{if value.isChecked}} checked="checked" {{/if}}>'+
                      '<label class="tydic-radio-label {{if value.disabled}} disabled {{/if}} {{if value.isChecked}} active {{/if}}" for="{{options.cid+options._totalIndex+i}}" ></label>'+
                      '<span class="tydic-radio-span">{{value.text}}</span>'+
                    '</li>'+
                  '{{/each}}'
     };
     //日期的配置项
     ...
     var initHtml = function ($dom) {
         ...
         options.isRequest && loadRadio($dom,{params:options["params"]});
     ...
     var parseOptionsEvent = function($dom){
       $dom.on("click",".tydic-radio-label",function(event){
         var onCheckFn,
             checkFn,
             options = $dom.data("options"),
             $this = $(this),
             checkVal = "",
             $input = $this.siblings(".tydic-radio"), //找到对应的单选框
             $span = $this.siblings(".tydic-radio-span");//找到对应的文本元素
         ...
         if(onCheckFn){
           ...
           if(checkVal){
             $this.closest(".cb-tpl-radio-list").find(".tydic-radio-label").removeClass("active");
             $this.closest(".cb-tpl-radio-list").find(".tydic-radio").prop("checked",false);
           ...
           checkFn && checkFn.call(this,options,$input.prop("checked"),$input.val(),$span.text());
           ...
           checkFn && checkFn.call(this,options,$this.hasClass("active"),$input.val(),$span.text());
           return false;
         }
     ...
     //加载单选框
     var loadRadio = function($dom,obj){
       //处理表格配置
     ...
     //为单选框追加数据
     var appendRadio = function($dom,obj){
       //处理表格配置
       var options = paseOptions($dom, obj);
       var renderSuccess = getCallBackFn(options["renderSuccess"]);
       if(obj["data"]){
     ...
     var loadData = function($dom,returnData,isAppend){
       ...
       TplHelper.getTplByStr(tpl,function(render,str){
         ...
         if(isAppend === "append"){
           $dom.find(".cb-tpl-radio-list").append(html);
         }else{
           $dom.find(".cb-tpl-radio-list").html(html);
         }
       });
     }
     //向后台请求数据
     ...
     var unCheckAll = function($dom){
       $dom.find(".tydic-radio-label.active").each(function(index,value){
           $(this).removeClass("active");
           $(this).siblings(".tydic-radio").prop("checked",false);
       });
     };
     //对外暴露，用原有的参数重新加载单选框
     var reLoadRadio = function($dom){
       var options = $dom.data("options");
       var params = options["params"] || {};
       loadRadio($dom,{params:params});
     }
     //对外暴露 取消所有单选框的选中状态。
     return {
         initTag: initTag,
         ...
         getParams:getParams,
         loadRadio:loadRadio,
         appendRadio:appendRadio,
         reLoadRadio:reLoadRadio,
 ...
 //注册Cb-tpl-date组件
 ParsingHelper.registerComponent("cb-tpl-radio", CbTplRadio);
