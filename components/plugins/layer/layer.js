...
var layer = {
  ...
  alert: function(content, options, yes){
    //根据业务需要(operate:add)，让图标类型具体化，暂时封装如下三个："warn"，"success","error"
    switch (options.icon){
      case 'warn':
            options.icon = 0;
            break;
  ...
  msg: function(content, options, end){ //最常用提示层
    //根据业务需要(operate:add)，让图标类型具体化，暂时封装如下三个："warn"，"success","fail"
    switch (options.icon){
      case 'warn':
        options.icon = 0;
        break;
