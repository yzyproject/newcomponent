/*
 * cb-tpl-radio-demo.js
 * author： niulei
 * Time: 2017.9.24
 * description: cb-tpl-radio-demo页面的业务逻辑js，存放此页面的业务逻辑。
 */
$(function(){
  console.log("进入页面JS");
...
var page = (function(){
  ...
  //单击单选框时的回调函数，return true选中 return false 不选中
  model.onCheck = function(options,state,value,text){
    console.log("调用单击单选框的回调函数",options,state,value,text);
    return !state;
  }
  //选择状态变更后的回调函数
  model.checkFn = function(options,state,value,text) {
    console.log("调用单选框状态变更的回调函数",options,state,value,text);
  ...
  fn.loadtheData = function(params){
    CbTplRadio.loadRadio($(this),{params:params});
  }
  //通过传入data来加载单选框数据
  fn.loadtheData2 = function(params) {
    CbTplRadio.loadRadio($(this),{data:{outputParamList:[
      {
        "text1":"类型1",
        "value1":"01",
        "disabled":true,
        "description":"disabled 这个字段为true时，单选框禁止点击"
      },
      {
        "text1":"类型二er",
        "value1":"02",
  ...
  fn.appendtheData = function(params){
    CbTplRadio.appendRadio($(this),{params:params});
  }
  //重新加载数据
  fn.reLoadFn = function(params){
    CbTplRadio.reLoadRadio($(this));
  }
  //取消所有选中框的选中状态
  fn.unCheckAll = function(params){
    CbTplRadio.unCheckAll($(this));
  }
  //事件函数绑定位置
